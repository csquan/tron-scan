...
class Tronapi:
    ...
    def freeze_balance(
        ...
        if (
            ...
            and keys.to_hex_address(receiver) != payload["owner_address"]
        ):
    ...
    def unfreeze_balance(
        self, owner: str = None, resource: str = "ENERGY", receiver: str = None
    ):
        payload = {
            "resource": resource,
        }
    ...
    # 投票
    def vote_witness_account(
        self, owner: str = None, vote_address: str = None, vote_count: int = 1
    ):
        payload = {
            "votes": [
                {
    ...
    # 查看一个账户给哪些账户代理了资源.
    def GetDelegatedResourceAccountIndex(
        self,
    ):
        payload = {
        ...
        response = self.request(
            action="/wallet/getdelegatedresourceaccountindex", data=payload
        )
        return response
    ...
    def getWalletSolidityBlockByNum(self, num):
        response = self.request(
            action="/walletsolidity/getblockbynum", data={"num": num}
        )
        return response

    def getTxInfoByNum(self, num):
        response = self.request(
            action="/walletsolidity/gettransactioninfobyblocknum", data={"num": num}
        )
        return response
    ...
    def getContractInfo(self, name, contract_address):
        response = self.request(
            action="/wallet/triggerconstantcontract",
            data={
